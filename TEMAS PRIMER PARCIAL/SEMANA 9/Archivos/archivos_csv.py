...
def leer_csv(path: str, modo: str) -> list[dict]:
    ...
    with open(path, modo) as mi_csv:
        # guardo cada linea de texto en una lista
        lista_de_renglones = mi_csv.readlines().copy()
        #al lo que contiene el  indice cero le saco \n y corto el str en las "," y lo guardo una una lsita de claves
        lista_claves = lista_de_renglones[0].replace('\n', '').split(',')
        # recorro lista de renglones a partir del segundo renglon
    
        for linea in lista_de_renglones[1:]:
            #  le saco \n y corto el str en las "," y lo guardo una una lsita de HEROES 
            lista_datos_de_heroe = linea.replace('\n', '').split(',')
            heroe_actual = {}
            # lsita claves y lista datos heroes tiene la misma extencion y con lo cual tienen el misnmo indicie
            for i in range(len(lista_claves)):
                #mediante listas paralelas armo el par clave-valor para cada heroe
                par_clave_valor = {lista_claves[i] : lista_datos_de_heroe[i]}
                # actualizo el diccionario con cada heroe nuevo
                heroe_actual.update(par_clave_valor)
                # lo agrego a la lista de heroes
